        ...
        **/
        ...
        static public function assertEqualsArrays(...args:Array):void {
            ...
            for (var i : int = 0; i < expected.length; i++) { 
				...
				{
					assertEqualsArrays(expected[i], actual[i]);  
				}                                                 
				else if(isVector( expected[i] ) )
				{
					assertEqualsVectors(expected[i], actual[i]);
				}                                               
				else
				{
		...
		static public function assertEqualsVectors(...args:Array):void {
			...
			assertEqualsArrays(message, expected_array, actual_array);
		}
		...
		static private function isVector(item:*):Boolean
		{
			return (getQualifiedClassName(item).substr(0, 11) == '__AS3__.vec');
		}
    }
}